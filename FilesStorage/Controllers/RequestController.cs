...
namespace FilesStorage.Controllers
{
    ...
    public class RequestController : ControllerBase
    {
        ...
        public ActionResult GetFile(string path)
        {
            ...
            else if (System.IO.File.Exists(fullPath)) // Path to file, for downloading
            {
                try
                {
        ...
        public ActionResult GetFileHeader(string path)
        {
            ...
            try
            {
                if (System.IO.File.Exists(fullPath))
                {
                    ...
                    {
                        Response.Headers.Add(item.Key, item.Value); // key - name, lastdata...
                    }
        ...
        public ActionResult PutFile(IFormFile file, string path)
        {
            ...
            try
            {
                ...
                if (Request.Headers.ContainsKey(PutHeaderCopyFrom))
                {
                    ...
                    else if (resCode == FileProcessing.PutCodes.WrongFileName)
                    {
                        return BadRequest(); 
                    } 
                    else
